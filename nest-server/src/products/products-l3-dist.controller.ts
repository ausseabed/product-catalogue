...
export class ProductsL3DistController extends ProductsController<ProductL3Dist, ProductL3DistDto>{
  ...
  create (@Body(new ClassValidationPipe()) product: ProductL3DistDto) {
    return this.productsService.create<ProductL3Dist, ProductL3DistDto>(this.productType, product);
  }
  @Put(':productId')
  @ApiBadRequestResponse({ description: 'Could not find the survey' })
