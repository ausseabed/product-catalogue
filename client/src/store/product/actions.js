...
export async function fetchData ({ commit }, recordId) {
  try {
    const response = await axios.get('/api/product/' + encodeURI(recordId))
...
export async function removeProduct ({ commit, dispatch }, productId) {
  try {
    var response = await axios.delete('/api/product/' + encodeURI(productId))
