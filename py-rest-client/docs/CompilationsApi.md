...
> list[Compilation] compilations_controller_find_all(snapshot_date_time=snapshot_date_time)
...
with product_catalogue_py_rest_client.ApiClient(configuration) as api_client:
    ...
    try:
        api_response = api_instance.compilations_controller_find_all(snapshot_date_time=snapshot_date_time)
        pprint(api_response)
    except ApiException as e:
